...
function useCollectionInfosImpl (ids?: BN[]): CollectionInfo[] | undefined {
  ...
  const metadata = useCall<[[BN[]], Option<PalletUniquesCollectionMetadata>[]]>(api.query.substrateUniques.classMetadataOf.multi, [ids], QUERY_OPTS);
  const details = useCall<[[BN[]], Option<PalletUniquesCollectionDetails>[]]>(api.query.substrateUniques.class.multi, [ids], QUERY_OPTS);
